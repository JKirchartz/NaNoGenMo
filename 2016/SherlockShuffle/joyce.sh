...
# Usage:
#
# $>joyce.sh output.txt [first|last|min|max|avg|avg2]
#
...
if [ -z "$1" ]; then
	echo "Usage:"
	echo "joyce.sh output.txt [first|last|min|max|avg|avg2]"
	...
	if [ ! -f ./corpora/joyce.corpus ]; then
		...
		cat ./corpora/*.txt > ./corpora/joyce.corpus
		echo "cleaning corpora....."
		vim ./corpora/joyce.corpus -S cleanGcorpora.vim
	...
	if [ -z "$2" ];then
		phrasechain.sh < ./corpora/joyce.corpus > tmp.txt
	else
		phrasechain.sh "$2" < ./corpora/joyce.corpus > tmp.txt
	fi
